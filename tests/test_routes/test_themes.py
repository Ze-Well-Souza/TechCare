...
def test_theme_persistence_for_logged_users(client):
    ...
    with patch('app.routes.main.current_user') as mock_current_user:
        ...
        # Adiciona tema escuro nas preferências
        response = client.post('/api/set_theme', json={'theme': 'dark'})
        assert response.status_code == 200
        ...
        # Verifica se a preferência está na sessão
        with client.session_transaction() as sess:
            assert sess['theme'] == 'dark'
...
def test_theme_default_system_preference(client):
    ...
    # Verifica se alguma das classes de tema escuro está presente ou se tem data-theme="dark"
    dark_theme_indicators = [
        'dark-theme' in body.get('class', []), 
        'dark-mode' in body.get('class', []), 
        body.get('data-theme') == 'dark',
        'theme-dark' in body.get('class', [])
    ]
    
    assert any(dark_theme_indicators), "Tema escuro não foi aplicado com base na preferência do navegador"
    
    # Simula cabeçalho que prefere tema claro
    headers = {'Accept': 'text/html', 'Sec-CH-Prefers-Color-Scheme': 'light'}
    
    # Limpa a sessão novamente
    with client.session_transaction() as sess:
        if 'theme' in sess:
            del sess['theme']
    ...
    # Verifica se a sessão tem o tema claro definido
    with client.session_transaction() as sess:
        assert 'theme' in sess
        assert sess['theme'] == 'light'
